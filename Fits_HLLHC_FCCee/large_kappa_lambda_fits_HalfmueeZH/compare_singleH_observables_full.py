...
# model_specs = ["fits_full", "fits"]
model_specs = ["fits_full", "fits_full"]

# Open the input file in read mode and output file in write mode
# working_dir_UVmodel = "../different_scenario_fits/"
working_dir = "./"
compare_dir = "../large_kappa_lambda_fits/"
compare_dir = "../large_kappa_lambda_fits_noLoopH3d6Quad/"
...
subprocess.run(["mkdir", "-p", f"{working_dir}comparison_plots/results_{model_specs[0]}"])
...
for ind, datasets in enumerate([files, files_compare]):
    ...
    for k in range(len(param_breaks) - 1):
        ...
        plot_obs(fig,
                ...
                file_name=working_dir + f'comparison_plots/results_{model_specs[0]}/mean_stddev_{k}.pdf',
                not_first_plot = bool(ind)
                )
        
        if ind == 1:
            plt.savefig(working_dir + f'comparison_plots/results_{model_specs[0]}/mean_stddev_{k}.pdf')
...
with open(f"comparison_plots/results_{model_specs[0]}/kappa_lambda_means_std_dev.txt", "w") as kappa_lambda_output:

    for scenario in scenarios:
        for obs in copy_obs:
            subprocess.run(["cp", f"{working_dir}{scenario}/results_{model_specs[0]}/Observables/{obs}.pdf",
                        f"{working_dir}comparison_plots/results_{model_specs[0]}/{obs}_{scenario}.pdf"])
