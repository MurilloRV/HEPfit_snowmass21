...
# BP_New_Names=("BPBnew_"{0..13})
BP_New_Names=("BPBnew_0" "BPBnew_1" "BPBnew_2" "BPBnew_4")
# BP_New_Names=()
...
# Exclusive flag
no_BSM="false" # Excludes the full BSM contribution to the k_Zh coupling
...
WFR_kala2_input="false" # Include the WFR contribution, proportional to kappa_lambda**2, into the Z2SSM ZH cross-section prediction
WFR_kala2_input_all="false" # Include the WFR contribution, proportional to kappa_lambda**2, into the Z2SSM predictions for all the XS and BR
use_HEPfit_C1_values_WFR_kala2_input_all="false" # Use the HEPfit C1 values, instead of the Z2SSM values. Activates WFR_kala2_input_all as well
...
Z2SSM_SCENARIOS=('Z2SSM_FCCee240_FCCee365')
...
for BP_Name in "${BP_Names_Total[@]}"; do
    ...
    for ((j=0; j<${#Z2SSM_SCENARIOS[@]}; j++)); do
        ...
        cp ${ORIGINAL_PATH}/${Z2SSM_SCENARIOS[j]}/Globalfits/AllOps/model_fits.conf Globalfits/AllOps/${MODEL_CONF_FILE}.conf
        # cp ${ORIGINAL_PATH}/${Z2SSM_SCENARIOS[j]}/Globalfits/AllOps/model_fits_small_priors.conf Globalfits/AllOps/${MODEL_CONF_FILE}_small_priors.conf
        ...
        if [[ "${Z2SSM_SCENARIOS[j]}" == "Z2SSM_FCCee240_FCCee365_HLLHClambda" ]]; then
