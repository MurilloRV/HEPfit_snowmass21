...
# Suffix for the model configuration file name, to distinguish between different setups of nuisance parameters. If empty, the default name will be used.
# model_conf_NP_suffix=""  
...
quadratic_func() {
    ...
    local b="$3"
    local c="$4"
    printf "%.20f" "$(echo "$a * $x * $x + $b * $x + $c" | bc)"
}
...
get_klam_prediction() {
    local bp_name="$1"
    if [ "$bp_name" == "BP_lambda1" ]; then
        echo "1.1000242642433875"
    elif [ "$bp_name" == "BPB_2" ]; then
        echo "2.3867362274064843"
    elif [ "$bp_name" == "BPB_4" ]; then
        echo "3.3446699219962595"
    elif [ "$bp_name" == "BPB_6" ]; then
        echo "4.332584967850238"
    else
        echo "Error: Unknown BP name '$bp_name'. Cannot determine kappa_lambda prediction."
        return 1
    fi
}
...
setup_fits() {
    ...
    if [ "$use_new_NPs" == "true" ]; then 
        ...
        # if [[ "$model_conf_NP_suffix" != "" ]]; then
        #     MODEL_CONF_FILE="${MODEL_CONF_FILE}_${model_conf_NP_suffix}"
        # else
        ...
        if [[ "$UseKlamDependentUncertainties" == "true" ]]; then
            ...
            MODEL_CONF_FILE="${MODEL_CONF_FILE}_klam_dependent${model_conf_NP_suffix}" # Commented out the following lines, file suffix was too long
        ...
        elif [[ "$UseBPDependentUncertainties" == "true" ]]; then
            if [[ "$theoerr_FCCee240_input" != "DEFAULT" || "$theoerr_FCCee365_input" != "DEFAULT" ]]; then
                echo "Warning: using BP-dependent uncertainties, but theoerr_FCCee240_input and theoerr_FCCee365_input are not set to DEFAULT (1.0)."
            fi
            MODEL_CONF_FILE="${MODEL_CONF_FILE}_BP_dependent${model_conf_NP_suffix}"
            # MODEL_CONF_FILE="${MODEL_CONF_FILE}_a240_${theoerr_FCCee240_function_x2_coef_path}"
            # MODEL_CONF_FILE="${MODEL_CONF_FILE}_b240_${theoerr_FCCee240_function_x1_coef_path}"
            # MODEL_CONF_FILE="${MODEL_CONF_FILE}_c240_${theoerr_FCCee240_function_x0_coef_path}"
            # MODEL_CONF_FILE="${MODEL_CONF_FILE}_a365_${theoerr_FCCee365_function_x2_coef_path}"
    ...
    if [[ "$ASIMOV" == "true" ]]; then
        fit_dir="${BP_Name}/${scenario}"
    else
        fit_dir="${BP_Name}/${scenario}/toy_fits"
        MODEL_CONF_FILE="${MODEL_CONF_FILE}_toyfit${fit_idx}"
    fi

    mkdir -p "${fit_dir}/Globalfits/AllOps"
    cd "${fit_dir}"
    cp ${ORIGINAL_PATH}/${scenario}/*.conf .
    cp ${ORIGINAL_PATH}/${scenario}/Globalfits/AllOps/d6Ops_corr.conf Globalfits/AllOps/
    cp ${ORIGINAL_PATH}/${scenario}/Globalfits/AllOps/model_fits.conf Globalfits/AllOps/${MODEL_CONF_FILE}.conf
    # cp ${ORIGINAL_PATH}/${scenario}/Globalfits/AllOps/model_fits_small_priors.conf Globalfits/AllOps/${MODEL_CONF_FILE}_small_priors.conf
    ...
    if [[ "$use_new_NPs" == "true" ]]; then
        ...
        if [[ "$UseKlamDependentUncertainties" == "true" ]]; then
            ...
            if [[ "$UseBPDependentUncertainties" == "true" ]]; then
                NEW_NP_CONF="${NEW_NP_CONF}_BP_dependent"
                NEW_NP_CONF="${NEW_NP_CONF}_a240_${theoerr_FCCee240_function_x2_coef_path}"
                NEW_NP_CONF="${NEW_NP_CONF}_b240_${theoerr_FCCee240_function_x1_coef_path}"
                NEW_NP_CONF="${NEW_NP_CONF}_c240_${theoerr_FCCee240_function_x0_coef_path}"
                NEW_NP_CONF="${NEW_NP_CONF}_a365_${theoerr_FCCee365_function_x2_coef_path}"
            ...
            elif [[ "$theoerr_FCCee240_function_x2_coef_path" != "0.0" || 
        ...
        if [[ "$UseBPDependentUncertainties" == "true" ]]; then
            klam_value=$(get_klam_prediction "$BP_Name")
            theoerr_FCCee240_BP=$(quadratic_func "$klam_value" "$theoerr_FCCee240_function_x2_coef_input" "$theoerr_FCCee240_function_x1_coef_input" "$theoerr_FCCee240_function_x0_coef_input")
            theoerr_FCCee365_BP=$(quadratic_func "$klam_value" "$theoerr_FCCee365_function_x2_coef_input" "$theoerr_FCCee365_function_x1_coef_input" "$theoerr_FCCee365_function_x0_coef_input")

            theoerr_FCCee240=$(printf "%.20f" "$(echo "$theoerr_FCCee240_BP * $theoerr_FCCee240" | bc)")
            theoerr_FCCee365=$(printf "%.20f" "$(echo "$theoerr_FCCee365_BP * $theoerr_FCCee365" | bc)")
        fi
    ...
    HIGGS_HLLHC_CONF="ObservablesHiggs_HLLHC_SM_kappa_scaled"
    ...
    HIGGS_240_CONF="${HIGGS_240_CONF}_kappa_scaled"
    HIGGS_365_CONF="${HIGGS_365_CONF}_kappa_scaled"
    ...
    if [ "$no_HLLHC_Higgs" == "true" ]; then
        ...
        sed -i "\/${HIGGS_HLLHC_CONF}.*/c\\# IncludeFile ${HIGGS_HLLHC_CONF}.conf" ${HIGGS_CONF}.conf
    ...
    if [ "$with_HEPfit_EWPOs" == "true" ]; then
        ...
        if [[ "${scenario}" != "IDM_FCCee240" ]]; then sed -i "\/IncludeFile ${VV_OO_CONF_FILE_FCCee_365}.conf/c\\IncludeFile ${NEW_VV_OO_CONF_FILE_FCCee_365}.conf" ${NEW_VV_OO_CONF_FILE}.conf; fi
    ...
    if [[ "$no_1L_BSM_sqrt_s" == "true" || 
        ...
        for FLAG in "${FLAG_ARRAY[@]}"; do
            if [ "${!FLAG}" == "true" ]; then
                ...
                NEW_HIGGS_240_CONF="${HIGGS_240_CONF}_${FLAG}"
                sed -i "\/${HIGGS_240_CONF}.conf/c\\IncludeFile ${NEW_HIGGS_240_CONF}.conf" ${HIGGS_CONF}.conf
                HIGGS_240_CONF="${NEW_HIGGS_240_CONF}"
                if [[ "${scenario}" != "IDM_FCCee240" ]]; then
                    NEW_HIGGS_365_CONF="${HIGGS_365_CONF}_${FLAG}"
                    sed -i "\/${HIGGS_365_CONF}.conf/c\\IncludeFile ${NEW_HIGGS_365_CONF}.conf" ${HIGGS_CONF}.conf
                    HIGGS_365_CONF="${NEW_HIGGS_365_CONF}"
                fi
                if [[ "$no_HLLHC_Higgs" != "true" ]]; then
                    NEW_HIGGS_HLLHC_CONF="${HIGGS_HLLHC_CONF}_${FLAG}"
                    sed -i "\/${HIGGS_HLLHC_CONF}.conf/c\\IncludeFile $NEW_HIGGS_HLLHC_CONF.conf" ${HIGGS_CONF}.conf
    ...
    if [[ "$ASIMOV" != "true" ]]; then
        NEW_HIGGS_CONF="${HIGGS_CONF}_toyfit${fit_idx}"
        NEW_HIGGS_HLLHC_CONF="${HIGGS_HLLHC_CONF}_toyfit${fit_idx}"
        NEW_HIGGS_240_CONF="${HIGGS_240_CONF}_toyfit${fit_idx}"
        NEW_HIGGS_365_CONF="${HIGGS_365_CONF}_toyfit${fit_idx}"

        NEW_EWPO_CONF_FILE="${EWPO_CONF_FILE}_toyfit${fit_idx}"
        NEW_EWPO_CONF_FILE_CURRENT="${EWPO_CONF_FILE_CURRENT}_toyfit${fit_idx}"
        NEW_EWPO_CONF_FILE_HLLHC="${EWPO_CONF_FILE_HLLHC}_toyfit${fit_idx}"
        NEW_EWPO_CONF_FILE_FCCee_Zpole="${EWPO_CONF_FILE_FCCee_Zpole}_toyfit${fit_idx}"
        NEW_EWPO_CONF_FILE_FCCee_WW="${EWPO_CONF_FILE_FCCee_WW}_toyfit${fit_idx}"

        NEW_VV_OO_CONF_FILE="${VV_OO_CONF_FILE}_toyfit${fit_idx}"
        NEW_VV_OO_CONF_FILE_FCCee_161="${VV_OO_CONF_FILE_FCCee_161}_toyfit${fit_idx}"
        NEW_VV_OO_CONF_FILE_FCCee_240="${VV_OO_CONF_FILE_FCCee_240}_toyfit${fit_idx}"
        NEW_VV_OO_CONF_FILE_FCCee_365="${VV_OO_CONF_FILE_FCCee_365}_toyfit${fit_idx}"
        NEW_VV_OO_CONF_FILE_aTGC_CURRENT="${VV_OO_CONF_FILE_aTGC_CURRENT}_toyfit${fit_idx}"
        NEW_VV_OO_CONF_FILE_aTGC_HLLHC="${VV_OO_CONF_FILE_aTGC_HLLHC}_toyfit${fit_idx}"


        cp ${HIGGS_CONF}.conf ${NEW_HIGGS_CONF}.conf
        sed -i "\/IncludeFile ..\/..\/ObservablesHiggs.*/c\\IncludeFile ../../${NEW_HIGGS_CONF}.conf " Globalfits/AllOps/${MODEL_CONF_FILE}.conf
        sed -i "\/IncludeFile ..\/..\/ObservablesHiggs.*/c\\IncludeFile ../../${NEW_HIGGS_CONF}.conf " Globalfits/AllOps/${MODEL_CONF_FILE}_small_priors.conf
        HIGGS_CONF="${NEW_HIGGS_CONF}"

        cp ${EWPO_CONF_FILE}.conf ${NEW_EWPO_CONF_FILE}.conf
        sed -i "\/IncludeFile ..\/..\/ObservablesEW.*/c\\IncludeFile ../../${NEW_EWPO_CONF_FILE}.conf " Globalfits/AllOps/${MODEL_CONF_FILE}.conf
        sed -i "\/IncludeFile ..\/..\/ObservablesEW.*/c\\IncludeFile ../../${NEW_EWPO_CONF_FILE}.conf " Globalfits/AllOps/${MODEL_CONF_FILE}_small_priors.conf
        EWPO_CONF_FILE="${NEW_EWPO_CONF_FILE}"

        cp ${VV_OO_CONF_FILE}.conf ${NEW_VV_OO_CONF_FILE}.conf
        sed -i "\/IncludeFile ..\/..\/ObservablesVV.*/c\\IncludeFile ../../${NEW_VV_OO_CONF_FILE}.conf " Globalfits/AllOps/${MODEL_CONF_FILE}.conf
        sed -i "\/IncludeFile ..\/..\/ObservablesVV.*/c\\IncludeFile ../../${NEW_VV_OO_CONF_FILE}.conf " Globalfits/AllOps/${MODEL_CONF_FILE}_small_priors.conf
        VV_OO_CONF_FILE="${NEW_VV_OO_CONF_FILE}"


        sed -i "\/IncludeFile ${HIGGS_240_CONF}.conf/c\\IncludeFile ${NEW_HIGGS_240_CONF}.conf" ${HIGGS_CONF}.conf
        if [[ "${scenario}" != "IDM_FCCee240" ]]; then sed -i "\/IncludeFile ${HIGGS_365_CONF}.conf/c\\IncludeFile ${NEW_HIGGS_365_CONF}.conf" ${HIGGS_CONF}.conf; fi
        if [[ "$no_HLLHC_Higgs" != "true" ]]; then sed -i "\/IncludeFile ${HIGGS_HLLHC_CONF}.conf/c\\IncludeFile ${NEW_HIGGS_HLLHC_CONF}.conf" ${HIGGS_CONF}.conf; fi

        if [ "$modify_all_ewpos" == "true" ]; then sed -i "\/IncludeFile ${EWPO_CONF_FILE_CURRENT}.conf/c\\IncludeFile ${NEW_EWPO_CONF_FILE_CURRENT}.conf" ${EWPO_CONF_FILE}.conf; fi
        sed -i "\/IncludeFile ${EWPO_CONF_FILE_HLLHC}.conf/c\\IncludeFile ${NEW_EWPO_CONF_FILE_HLLHC}.conf" ${EWPO_CONF_FILE}.conf
        sed -i "\/IncludeFile ${EWPO_CONF_FILE_FCCee_Zpole}.conf/c\\IncludeFile ${NEW_EWPO_CONF_FILE_FCCee_Zpole}.conf" ${EWPO_CONF_FILE}.conf
        sed -i "\/IncludeFile ${EWPO_CONF_FILE_FCCee_WW}.conf/c\\IncludeFile ${NEW_EWPO_CONF_FILE_FCCee_WW}.conf" ${EWPO_CONF_FILE}.conf
...
for BP_Name in "${BP_Names_Total[@]}"; do
    ...
    mkdir -p "${BP_Name}"
    for scenario in "${IDM_SCENARIOS[@]}"; do

        if [[ "$ASIMOV" == "true" ]]; then
            setup_fits $BP_Name $scenario $ASIMOV 0
        else
            for ((fit_idx=0; fit_idx<$N_EXPS; fit_idx++)); do
                setup_fits $BP_Name $scenario $ASIMOV $fit_idx
            done
        fi
