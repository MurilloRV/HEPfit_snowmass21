...
for i, BP in enumerate(BPs):
    for scenario in scenarios:
        ...
        for k in range(len(param_breaks) - 1):
            ...
            ax.set_title(plot_title[BP][scenario], fontsize=12)
            plt.tight_layout()   # Makes sure labels are not cut off
            plt.savefig(working_dir + f'comparison_plots/results_{results_dir}/pull_pars_{BP}_{scenario}_compare_{k}.pdf')
