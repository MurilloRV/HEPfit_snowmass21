...
for BP_Name in "${BP_Names_Total[@]}"; do
    ...
    for ((j=0; j<${#IDM_SCENARIOS[@]}; j++)); do
        ...
        # Using the HEPfit SMEFT expression for sigma_Zh
        elif [ "$smeft_formula" == "true" ]; then
            ...
            cp ${HIGGS_CONF}.conf ${HIGGS_CONF}_smeft_formula.conf 
            NEW_HIGGS_240="IncludeFile ObservablesHiggs_FCCee_240_SM_kappa_scaled_smeft_formula.conf"
            sed -i "\/ObservablesHiggs_FCCee_240_SM_kappa_scaled.conf/c\\$NEW_HIGGS_240" ${HIGGS_CONF}_smeft_formula.conf
            if [[ "${IDM_SCENARIOS[j]}" != "IDM_FCCee240" ]]; then
                NEW_HIGGS_365="IncludeFile ObservablesHiggs_FCCee_365_kappa_scaled_smeft_formula.conf"
                sed -i "\/ObservablesHiggs_FCCee_365_kappa_scaled.conf/c\\$NEW_HIGGS_365" ${HIGGS_CONF}_smeft_formula.conf
            fi
            NEW_HIGGS_HLLHC="IncludeFile ObservablesHiggs_HLLHC_SM_kappa_scaled_smeft_formula.conf"
            sed -i "\/ObservablesHiggs_HLLHC_SM_kappa_scaled.conf/c\\$NEW_HIGGS_HLLHC" ${HIGGS_CONF}_smeft_formula.conf

            NEW_MODEL_HIGGS="IncludeFile ../../${HIGGS_CONF}_smeft_formula.conf "
            ...
            python scale_observables_kappas.py --scenario ${IDM_SCENARIOS[j]} --bp ${BP_Name}
            python scale_observables_kappas.py --scenario ${IDM_SCENARIOS[j]} --bp ${BP_Name} --realistic
            python scale_observables_kappas.py --scenario ${IDM_SCENARIOS[j]} --bp ${BP_Name} --realistic --smeft_formula


        
        # Using the HEPfit SMEFT expression for sigma_Zh, including dkappaf**2 inside of the square root
        elif [ "$smeft_formula_sqrt" == "true" ]; then
            ...
            cp ${HIGGS_CONF}.conf ${HIGGS_CONF}_smeft_formula_sqrt.conf 
            NEW_HIGGS_240="IncludeFile ObservablesHiggs_FCCee_240_SM_kappa_scaled_smeft_formula_sqrt.conf"
            sed -i "\/ObservablesHiggs_FCCee_240_SM_kappa_scaled.conf/c\\$NEW_HIGGS_240" ${HIGGS_CONF}_smeft_formula_sqrt.conf
            if [[ "${IDM_SCENARIOS[j]}" != "IDM_FCCee240" ]]; then
                NEW_HIGGS_365="IncludeFile ObservablesHiggs_FCCee_365_kappa_scaled_smeft_formula_sqrt.conf"
                sed -i "\/ObservablesHiggs_FCCee_365_kappa_scaled.conf/c\\$NEW_HIGGS_365" ${HIGGS_CONF}_smeft_formula_sqrt.conf
            fi
            NEW_HIGGS_HLLHC="IncludeFile ObservablesHiggs_HLLHC_SM_kappa_scaled_smeft_formula_sqrt.conf"
            sed -i "\/ObservablesHiggs_HLLHC_SM_kappa_scaled.conf/c\\$NEW_HIGGS_HLLHC" ${HIGGS_CONF}_smeft_formula_sqrt.conf

            NEW_MODEL_HIGGS="IncludeFile ../../${HIGGS_CONF}_smeft_formula_sqrt.conf "
            ...
            python scale_observables_kappas.py --scenario ${IDM_SCENARIOS[j]} --bp ${BP_Name}
            python scale_observables_kappas.py --scenario ${IDM_SCENARIOS[j]} --bp ${BP_Name} --realistic
            python scale_observables_kappas.py --scenario ${IDM_SCENARIOS[j]} --bp ${BP_Name} --realistic --smeft_formula_sqrt




        # Using the HEPfit SMEFT expression for sigma_Zh, removing cross terms
        elif [ "$smeft_formula_no_cross" == "true" ]; then
            ...
            cp ${HIGGS_CONF}.conf ${HIGGS_CONF}_smeft_formula_no_cross.conf 
            NEW_HIGGS_240="IncludeFile ObservablesHiggs_FCCee_240_SM_kappa_scaled_smeft_formula_no_cross.conf"
            sed -i "\/ObservablesHiggs_FCCee_240_SM_kappa_scaled.conf/c\\$NEW_HIGGS_240" ${HIGGS_CONF}_smeft_formula_no_cross.conf
            if [[ "${IDM_SCENARIOS[j]}" != "IDM_FCCee240" ]]; then
                NEW_HIGGS_365="IncludeFile ObservablesHiggs_FCCee_365_kappa_scaled_smeft_formula_no_cross.conf"
                sed -i "\/ObservablesHiggs_FCCee_365_kappa_scaled.conf/c\\$NEW_HIGGS_365" ${HIGGS_CONF}_smeft_formula_no_cross.conf
            fi
            NEW_HIGGS_HLLHC="IncludeFile ObservablesHiggs_HLLHC_SM_kappa_scaled_smeft_formula_no_cross.conf"
            sed -i "\/ObservablesHiggs_HLLHC_SM_kappa_scaled.conf/c\\$NEW_HIGGS_HLLHC" ${HIGGS_CONF}_smeft_formula_no_cross.conf

            NEW_MODEL_HIGGS="IncludeFile ../../${HIGGS_CONF}_smeft_formula_no_cross.conf "
            ...
            python scale_observables_kappas.py --scenario ${IDM_SCENARIOS[j]} --bp ${BP_Name}
            python scale_observables_kappas.py --scenario ${IDM_SCENARIOS[j]} --bp ${BP_Name} --realistic
            python scale_observables_kappas.py --scenario ${IDM_SCENARIOS[j]} --bp ${BP_Name} --realistic --smeft_formula_no_cross



        # Using the HEPfit SMEFT expression for sigma_Zh, without the external-leg correction
        elif [ "$smeft_formula_external_leg" == "true" ]; then
            ...
            cp ${HIGGS_CONF}.conf ${HIGGS_CONF}_smeft_formula_external_leg.conf 
            NEW_HIGGS_240="IncludeFile ObservablesHiggs_FCCee_240_SM_kappa_scaled_smeft_formula_external_leg.conf"
            sed -i "\/ObservablesHiggs_FCCee_240_SM_kappa_scaled.conf/c\\$NEW_HIGGS_240" ${HIGGS_CONF}_smeft_formula_external_leg.conf
            if [[ "${IDM_SCENARIOS[j]}" != "IDM_FCCee240" ]]; then
                NEW_HIGGS_365="IncludeFile ObservablesHiggs_FCCee_365_kappa_scaled_smeft_formula_external_leg.conf"
                sed -i "\/ObservablesHiggs_FCCee_365_kappa_scaled.conf/c\\$NEW_HIGGS_365" ${HIGGS_CONF}_smeft_formula_external_leg.conf
            fi
            NEW_HIGGS_HLLHC="IncludeFile ObservablesHiggs_HLLHC_SM_kappa_scaled_smeft_formula_external_leg.conf"
            sed -i "\/ObservablesHiggs_HLLHC_SM_kappa_scaled.conf/c\\$NEW_HIGGS_HLLHC" ${HIGGS_CONF}_smeft_formula_external_leg.conf

            NEW_MODEL_HIGGS="IncludeFile ../../${HIGGS_CONF}_smeft_formula_external_leg.conf "
            ...
            python scale_observables_kappas.py --scenario ${IDM_SCENARIOS[j]} --bp ${BP_Name}
            python scale_observables_kappas.py --scenario ${IDM_SCENARIOS[j]} --bp ${BP_Name} --realistic
            python scale_observables_kappas.py --scenario ${IDM_SCENARIOS[j]} --bp ${BP_Name} --realistic --smeft_formula_external_leg



        
        # Using the HEPfit SMEFT expression for sigma_Zh, without the external-leg correction
        elif [ "$WFR_kala2_input" == "true" ]; then
            ...
            cp ${HIGGS_CONF}.conf ${HIGGS_CONF}_WFR_kala2_input.conf 
            NEW_HIGGS_240="IncludeFile ObservablesHiggs_FCCee_240_SM_kappa_scaled_WFR_kala2_input.conf"
            sed -i "\/ObservablesHiggs_FCCee_240_SM_kappa_scaled.conf/c\\$NEW_HIGGS_240" ${HIGGS_CONF}_WFR_kala2_input.conf
            if [[ "${IDM_SCENARIOS[j]}" != "IDM_FCCee240" ]]; then
                NEW_HIGGS_365="IncludeFile ObservablesHiggs_FCCee_365_kappa_scaled_WFR_kala2_input.conf"
                sed -i "\/ObservablesHiggs_FCCee_365_kappa_scaled.conf/c\\$NEW_HIGGS_365" ${HIGGS_CONF}_WFR_kala2_input.conf
            fi
            NEW_HIGGS_HLLHC="IncludeFile ObservablesHiggs_HLLHC_SM_kappa_scaled_WFR_kala2_input.conf"
            sed -i "\/ObservablesHiggs_HLLHC_SM_kappa_scaled.conf/c\\$NEW_HIGGS_HLLHC" ${HIGGS_CONF}_WFR_kala2_input.conf

            NEW_MODEL_HIGGS="IncludeFile ../../${HIGGS_CONF}_WFR_kala2_input.conf "
            ...
            python scale_observables_kappas.py --scenario ${IDM_SCENARIOS[j]} --bp ${BP_Name}
            python scale_observables_kappas.py --scenario ${IDM_SCENARIOS[j]} --bp ${BP_Name} --realistic
            python scale_observables_kappas.py --scenario ${IDM_SCENARIOS[j]} --bp ${BP_Name} --realistic --WFR_kala2_input
