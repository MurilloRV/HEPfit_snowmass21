...
# model_spec = "smallpriors"
# model_spec = "smallpriors_all_uncertainties"
model_spec = "SQUARED"
# model_spec = "smallpriors_all_uncertainties_HIGH"

# Open the input file in read mode and output file in write mode
working_dir = "./"
scenarios = ["SM_noHLLHClambda", 
             "SM_HLLHClambda", 
             "Z2SSM_noHLLHClambda", 
             "Z2SSM_HLLHClambda"
             ]
plot_titles = [r"SM Central values, no HL-LHC $\lambda_{hhh}$ constraint",
          r"SM Central values, with HL-LHC $\lambda_{hhh}$ constraint",
          r"Z2SSM Central values, no HL-LHC $\lambda_{hhh}$ constraint",
          r"Z2SSM Central values, with HL-LHC $\lambda_{hhh}$ constraint",
          ]
plot_labels = [r"SM, no $\kappa_\lambda$",
               r"SM, with $\kappa_\lambda$",
               r"$\mathbb{Z}_{2}$SSM, no $\kappa_\lambda$",
               r"$\mathbb{Z}_{2}$SSM, with $\kappa_\lambda$",
               ]
...
for file_path, scenario in zip(files, scenarios):
    ...
    with open(file_path, 'r') as file:
        ...
        for id, line in enumerate(lines):
            if line.startswith(" Results of the marginalization"):
                line_marg_start = id

        npar = len(parameters)
        # results_central68[scenario] = np.zeros((npar, 3))
        results_central68[scenario] = []
        for id, par in enumerate(parameters):
            # print(len(lines))
            # print(line_marg_start + 6 + 13*id)
            columns = lines[line_marg_start + 6 + 13*id].split()
            results_central68[scenario].append([float(columns[5]),    # Median
                                                float(columns[7]),    # Upper bound
                                                float(columns[9]),])  # Lower bound

    results[scenario] = np.array(results[scenario])
    results_central68[scenario] = np.array(results_central68[scenario])
...
#                     fmt='o', linewidth=2, capsize=6, label = plot_labels[i])
...
#     #plt.savefig(working_dir + f'comparison_plots/pull_plots_{k}.pdf')
...
### Medians and central 68% intervals

w = 1.0
dimw = w / 2
y_shift = np.linspace(0, -dimw, n_scenarios) 

parameter_order = [math.ceil(np.log10(par)) for par in results_central68[scenarios[0]][:, 1]]
parameter_order = np.array(parameter_order)

results_plot = results_central68
for scenario in scenarios:
    # results_plot[scenario][:,0] = results_plot[scenario][:,0] - results_plot[scenarios[0]][:,0]
    results_plot[scenario] = [par/10.**parameter_order[i] for i, par in enumerate(results_central68[scenario])]
    results_plot[scenario] = np.array(results_plot[scenario])
#print(results_plot)
...
for k in range(len(param_breaks) - 1):
    ...
    for i, scenario in enumerate(scenarios):
        ...
        results[:, 0] = results[:, 0] - results_plot[scenarios[0]][:, 0]
        ax.errorbar(results[param_breaks[k]:param_breaks[k+1], 0],
                    -y+y_shift[i], xerr=(results[param_breaks[k]:param_breaks[k+1], 2],
                                         results[param_breaks[k]:param_breaks[k+1], 1]), 
                    fmt='o', linewidth=2, capsize=6, label = plot_labels[i])
    ...
    ax.tick_params(axis='x', size=10, labelsize=12)
    ax.tick_params(axis='x', which='minor', size=6)
    ax.set_xlabel(r'Median and central 68% interval', fontsize=15)
    ax.legend(loc='best', fontsize=10)
    plt.tight_layout()   # Makes sure labels are not cut off
    plt.savefig(working_dir + f'comparison_plots/results_{model_spec}/median_68central_{k}.pdf')
